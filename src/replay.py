...
class ReplayBuffer:
    def __init__(self, capacity, state_size):
        ...
        self.size = 0
        self.states = np.zeros((capacity, state_size))
        self.actions = np.zeros((capacity, 1))
        self.rewards = np.zeros((capacity, 1))
        self.terminals = np.zeros((capacity, 1))
        self.next_states = np.zeros((capacity, state_size))
    ...
    def insert(self, transitions):
        ...
        for t in append_trans:
            ...
            self.actions[self.size] = t.actions
            self.rewards[self.size] = t.rewards
